...
const PaymentModal = ({ isOpen, onClose, onPaymentSuccess, amount, method }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-scale-in">
                {/* Header */}
                <div className="p-6 text-center border-b border-gray-100 dark:border-gray-700">
                    <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-4">
                        <Smartphone className="w-8 h-8 text-primary" />
                    </div>
                    <h2 className="text-xl font-bold text-text-main dark:text-white uppercase tracking-wider">
                        Mobile Payment Simulation
                    </h2>
                    <p className="text-text-secondary dark:text-gray-400 mt-1">
                        Simulating {method === 'mobile_banking' ? 'Telebirr/CBE Birr' : 'Bank Transfer'}
                    </p>
                </div>
                ...
                <div className="p-8">
                    {step === 'confirm' && (
                        <div className="space-y-6">
                            <div className="bg-bg-light dark:bg-gray-700/50 p-4 rounded-xl text-center">
                                <span className="text-sm text-text-secondary dark:text-gray-400 block mb-1">Amount to Pay</span>
                                <span className="text-3xl font-bold text-primary">ETB {amount.toLocaleString()}</span>
                            </div>

                            <div className="space-y-4">
                                <label className="block text-sm font-medium text-text-secondary dark:text-gray-400">
                                    Enter Simulated PIN (4 digits)
                                </label>
                        ...
                        ${pin.length >= i ? 'border-primary bg-primary/5' : 'border-gray-200 dark:border-gray-600'}`}
                                        >
                                            {pin.length >= i ? '•' : ''}
                                        </div>
                                    ))}
                                ...
                                <div className="grid grid-cols-3 gap-2 max-w-[240px] mx-auto mt-4">
                                    {[1, 2, 3, 4, 5, 6, 7, 8, 9, 'C', 0, 'OK'].map((key) => (
                                        <button
                                            ...
                                            className="h-12 rounded-lg bg-gray-50 dark:bg-gray-700 hover:bg-primary/10 
