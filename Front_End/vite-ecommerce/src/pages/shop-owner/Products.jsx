...
const Products = () => {
  const { user } = useAuth();
  const shopId = user?.shop_id || user?.id; // Fallback if shop_id isn't directly on user
  ...
  return (
    <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-text-secondary dark:text-gray-400 mt-2">
              Manage your shop's products and catalog
            </p>
          </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-border-default dark:border-gray-700 p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-border-default dark:border-gray-700">
          <Select
            label="Category"
            name="category_id"
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border-default dark:border-gray-700">
                ...
                <th className="text-left p-6 font-semibold text-text-main dark:text-gray-200">Stock</th>
              ...
              {loading ? (
                ...
                products.map((product) => (
                  <tr key={product.id} className="border-b border-border-default dark:border-gray-700 hover:bg-bg-light dark:hover:bg-gray-700">
                    ...
                    <td className="p-6">
                      <button onClick={() => handleToggleStatus(product.id, product.is_active)} className="flex items-center gap-2">
                        {product.is_active ? (
                          <>
                    ...
                    <td className="p-6">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => window.location.href = `/shop-owner/inventory/${product.id}`}
                ...
                <tr>
                  <td colSpan="7" className="p-12 text-center">
                    ...
                    <p className="text-text-secondary dark:text-gray-400">Add some products to your inventory</p>
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="p-6 border-t border-border-default dark:border-gray-700">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Button variant="secondary" onClick={() => handleFilterChange("page", pagination.page - 1)} disabled={pagination.page <= 1}>Previous</Button>
                <Button variant="secondary" onClick={() => handleFilterChange("page", pagination.page + 1)} disabled={pagination.page >= (pagination.totalPages || 1)}>Next</Button>
              </div>
            </div>
          </div>
        )}
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            <h2 className="text-xl font-semibold text-text-main dark:text-gray-200 mb-6">Add New Product</h2>
            <form onSubmit={handleAddProduct}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <Select
                  ...
                  options={[{ value: "", label: "Select Category" }, ...categoryOptions.slice(1)]}
                  required
                />
                <Input
                  label="Price (ETB)"
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium text-text-main dark:text-gray-200 mb-2">Description</label>
                <textarea
                  value={formData.description}
                  onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))}
                  rows={4}
                  className="w-full px-4 py-3 border border-border-default dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent resize-none bg-white dark:bg-gray-900 text-text-main"
                />
              </div>

              <div className="mb-6">
                <label className="block text-sm font-medium text-text-main dark:text-gray-200 mb-2">Product Image</label>
                <div className="border-2 border-dashed border-border-default dark:border-gray-700 rounded-lg p-8 text-center">
                  {formData.main_image ? (
                    <div>
                      <img src={URL.createObjectURL(formData.main_image)} alt="Preview" className="w-32 h-32 object-cover rounded-lg mx-auto mb-4" />
                      <p className="text-sm text-text-secondary mb-2">{formData.main_image.name}</p>
                      <button type="button" onClick={() => setFormData(prev => ({ ...prev, main_image: null }))} className="text-red-500 text-sm">Remove</button>
                    </div>
                  ) : (
                    <div>
                      <Upload className="w-12 h-12 text-text-muted mx-auto mb-4" />
                      <p className="text-text-secondary mb-2">Click to upload product image</p>
                      <input type="file" accept="image/*" onChange={handleImageChange} className="hidden" id="product-image" />
                      <label htmlFor="product-image" className="inline-block px-4 py-2 bg-primary text-white rounded-lg cursor-pointer hover:bg-primary/90">Choose File</label>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <Button variant="secondary" onClick={() => { setShowAddModal(false); resetForm(); }} className="flex-1">Cancel</Button>
              </div>
            </form>
          </div>
        </div>
